...
EL_DIST.grid(row=3, column=1, columnspan=4, **grid_dict)
...
def changeText():
    if save_button["text"] == "Stop":
        save_button["text"] = "Run"
        save_button.configure(bg="green", fg="black")
        button_action()
    else:
        save_button["text"] = "Stop"
        save_button.configure(bg="red", fg="white")
        button_action()


save_dir_button = Button(app, text="Save as", command=save_path_window)
save_dir_button.grid(row=7, column=3, sticky="e")

save_button = Button(app, text="Run", background="green", command=changeText)
...
datei_menu.add_command(label="Save settings", command=save_path_window)
